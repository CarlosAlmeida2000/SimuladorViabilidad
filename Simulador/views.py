...
def obtenerActividades(flujo_efectivo, id_tipo_cuenta):
    try:
        ...
        actividades = ActFinancieras.objects.filter(tipo_cuenta__id = id_tipo_cuenta).annotate(valor_mes1 = F('nombres')
            ).annotate(valor_mes2 = F('nombres')).annotate(valor_mes3 = F('nombres')).annotate(valor_mes4 = F('nombres')).annotate(valor_mes5 = F('nombres')
            ).annotate(valor_mes6 = F('nombres')).annotate(valor_mes7 = F('nombres')).annotate(valor_mes8 = F('nombres')).annotate(valor_mes9 = F('nombres')
            ).annotate(valor_mes10 = F('nombres')).annotate(valor_mes11 = F('nombres')).annotate(valor_mes12 = F('nombres')
            ...
            ).annotate(id_valor_mes10 = F('id')).annotate(id_valor_mes11 = F('id')).annotate(id_valor_mes12 = F('id')).values('id', 'nombres',
...
def vwGuardarActividad(request):
    try:
        ...
        # si el id es positivo significa que ya existe, entonces esa actividad se prepara para modificar
        if id_actividad > 0:
...
def vwGuardarValor(request):
    try:
        ...
        # si el id es positivo significa que ya existe, entonces ese valor se prepara para modificar
        if id_valor > 0:
...
def vwCalcularViabilidad(request):
    try:
        ...
        dias = parte_entera3
        return JsonResponse({'viabilidad': '1', 'tir': tir, 'van': van, 'razon_bc': razon_bc, 'anios': anios, 'meses': meses, 'dias': dias})
    except Exception as e: 
